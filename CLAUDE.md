...
- **CI/CD Workflow Changes** - ALWAYS consult [docs/development/ci-cd-pipeline.md](docs/development/ci-cd-pipeline.md) before modifying GitHub Actions workflows. The document describes the test execution strategy, release process, and dependencies between workflows. Releases are automated by `semantic-release` on merge to `main`; the release/changelog/version logic lives in `.releaserc.json`, not in shell.
- **Releases are automated** - `semantic-release` runs on merge to `main` and owns versioning. NEVER hand-edit the `version` in `package.json`, hand-edit `CHANGELOG.md`, or create `v*` git tags. `devel -> main` PRs MUST be merged as merge commits, never squashed, or version inference breaks. To preview a release: `GH_TOKEN=$(gh auth token) npx semantic-release --dry-run --no-ci --branches "$(git branch --show-current)"`.
- **PDF text lookup / Autosearch changes** - ALWAYS read [tests/pdf-match/README.md](tests/pdf-match/README.md) before touching the PDF text matcher (`app/src/modules/pdf-text-matcher.js`) or the Autosearch switch in the PDF Viewer. It documents the offline fetch → prepare → run → review → confirm pipeline and gold-case regression suite that any algorithm change must go through
- **Run the full test suite before finishing** - After finishing an implementation, and at the latest before exiting a worktree, ALWAYS run the full suite including E2E (`npm run test:unit` and `npm run test:e2e`), not just the tests for the touched code. E2E catches plugin install/start ordering and console-error regressions that unit tests miss.
- **Visually check targeted UI changes** - After a targeted UI change, run `node scripts/dev/ui-screenshot.js --out <scratchpad>/x.png [--click '<selector>']... [--mock '<url glob>=<json>']...` and view the PNG with the Read tool. It builds the demo container from the current checkout (`docker/demo-data`), logs in as `admin`/`admin`, performs the clicks, takes the screenshot and removes the container and image again. Use `--mock` for backend features the demo lacks (e.g. `'**/api/v1/llm/providers={"json":[...]}'` to simulate a configured LLM provider), `--base-url` to use an already running instance, `--no-build` to reuse the image. Run `--help` for all options. Keep screenshots in the scratchpad, not the repo.
- **Worktrees are for larger work only** - A worktree is never needed for small fixes, documentation changes or small isolated features; implement those directly on `devel` or in a feature branch.
