...
- **Backend plugin HTML templates** - NEVER inline long HTML strings in route files. Place HTML in the plugin's `html/` directory (e.g., `fastapi_app/plugins/<name>/html/view.html`) and load it with `load_plugin_html(__file__, "view.html")` from `fastapi_app.lib.plugin_tools`. This function reads the file and injects the sandbox client script automatically. NEVER put significant JavaScript inline in HTML templates — place it in a separate `.js` file in the same `html/` directory (auto-served at `/api/plugins/<name>/static/<file>.js`) and reference it with `<script type="module" src="...">`. This enables IDE type-checking and JSDoc type resolution
