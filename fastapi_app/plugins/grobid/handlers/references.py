"""Handler for GROBID processReferences API endpoint."""
...
class ReferencesHandler(GrobidHandler):
    """Handler for /api/processReferences endpoint."""

    def get_endpoint(self) -> str:
        return "/api/processReferences"

    def get_supported_variants(self) -> list[str]:
        return ["grobid.service.references"]

    def fetch_tei(self, pdf_path: str, grobid_server_url: str,
                  variant_id: str, flavor: str, options: Dict[str, Any]) -> str:
        """
        Fetch references TEI from GROBID processReferences endpoint.

        Returns TEI XML containing all bibliographical references from the document.
        """
        logger.info(f"Processing references: {pdf_path}")
