/**
 * Integration tests for the webdav-sync plugin.
 * @testCovers fastapi_app/plugins/webdav_sync/routes.py
 * @testCovers fastapi_app/plugins/webdav_sync/service.py
 * @testCovers fastapi_app/plugins/webdav_sync/remote_metadata.py
 *
 * Run via: node fastapi_app/plugins/webdav_sync/tests/run-integration-tests.js
 */

import { test, describe } from 'node:test';
import assert from 'node:assert';
import { login, authenticatedApiCall, authenticatedRequest } from '../../../../tests/api/helpers/test-auth.js';
...
describe('Sync and SSE API Integration Tests', { concurrency: 1 }, () => {
  ...
  test('GET /api/sync/status should return sync status with O(1) check', async () => {
    ...
    const response = await authenticatedApiCall(
      session.sessionId,
      '/api/plugins/webdav-sync/status',
      'GET',
      null,
      BASE_URL
    );
  ...
  test('Sync should download new remote files to local', async () => {
    ...
    // Sync to upload
    await authenticatedApiCall(session.sessionId, '/api/plugins/webdav-sync/sync', 'POST', { force: false }, BASE_URL);
  ...
  test('GET /api/sync/conflicts should return empty list when no conflicts', async () => {
    ...
    const response = await authenticatedApiCall(
      session.sessionId,
      '/api/plugins/webdav-sync/conflicts',
      'GET',
      null,
      BASE_URL
    );
  ...
  test('POST /api/sync/resolve-conflict should resolve with local_wins strategy', async () => {
    ...
    try {
      const response = await authenticatedRequest(
        session.sessionId,
        '/api/plugins/webdav-sync/resolve',
        'POST',
        {
          file_id: 'nonexistent',
