...
class AccessControlChecker:
    ...
    def check_document_access(
        ...
        """
        logger.debug(
            f"checking access for user={user}, "
        ...
        if not user:
            ...
            logger.debug(f"anonymous user, result={result}")
            return result

        username = user.get('username')
        logger.debug(f"user={username}, roles={user.get('roles', [])}")
        ...
        if permissions.visibility == 'private':
            ...
            if permissions.owner != username:
                logger.debug(
                    f"private document, owner={permissions.owner}, "
        ...
        if required_access == 'write':
            if permissions.editability == 'protected':
                ...
                if permissions.owner != username:
                    logger.debug(
                        f"protected document, owner={permissions.owner}, "
                        f"user={username}, write access denied"
                    )
                    return False

        logger.debug(
            f"allowing access - visibility={permissions.visibility}, "
