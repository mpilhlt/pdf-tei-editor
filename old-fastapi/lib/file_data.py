...
from .server_utils import get_data_file_path
from .cache_manager import mark_cache_clean
from .hash_utils import generate_file_hash, create_hash_mapping
...
def get_tei_metadata(file_path, logger=None):
    """
...
def get_version_name(file_path, logger=None):
    """
    ...
    try:
        ...
        if logger:
            logger.warning(f"XML Syntax Error in {file_path}: {str(e)}")
        return ""
...
def collect_and_cache_file_data(data_root: Path, db_dir: Path, logger=None):
    """
    ...
    Args:
        ...
        db_dir: Path to the database directory for caching
    ...
    file_data = _collect_file_metadata(data_root, logger)
    ...
    file_data, lookup_table = _shorten_hashes(file_data, lookup_table, logger)

    # Cache everything to disk
    _save_to_cache(db_dir / 'files.json', file_data, logger)
    _save_to_cache(db_dir / 'lookup.json', lookup_table, logger)
    _save_to_cache(db_dir / 'collections.json', collections_data, logger)

    if logger:
        logger.info(f"Cached file data: {len(file_data)} files, {len(lookup_table)} lookups, {len(collections_data)} collections")
        logger.debug(f"XML metadata parsed for {len(_metadata_cache)} files")
...
def _get_cached_metadata(file_path, logger=None):
    """
    ...
    try:
        tei_metadata = get_tei_metadata(file_path, logger)
        version_name = get_version_name(file_path, logger)
    except Exception as e:
        if logger:
            logger.debug(f"Failed to parse metadata for {file_path}: {e}")
...
def _collect_file_metadata(data_root: Path, logger=None):
    """
    ...
    for file_path in glob(f"{data_root}/**/*", recursive=True):
        ...
        file_type, file_id = _analyze_file(file_path, rel_path, data_root, logger)
        ...
        file_info = {
            ...
            'collection': _extract_collection_from_path(rel_path, data_root)
        }

        file_id_data[file_id][file_type].append(file_info)

    # Convert to final file list structure
    return _build_file_list(file_id_data, data_root, logger)

def _analyze_file(file_path, rel_path, data_root, logger=None):
    """
    ...
    if file_type == "xml":
        tei_metadata, _ = _get_cached_metadata(file_path, logger)
...
def _extract_collection_from_path(rel_path, data_root):
    """
...
def _build_file_list(file_id_data, data_root, logger=None):
    """
    ...
    for file_id, file_type_data in file_id_data.items():
        ...
        for file_type, files in file_type_data.items():
            for file_info in files:
                ...
                if is_version_file:
                    ...
                    version_entry = _build_version_entry(file_info, path_from_root, file_id, format_label=False, logger=logger)
        ...
        gold_variants = _build_gold_variants_array(file_id, file_type_data, logger)
        if gold_variants:
            file_dict['gold'] = gold_variants

        # Add top-level metadata from main XML file
        _add_top_level_metadata(file_dict, data_root, logger)
...
def _build_entry_with_metadata(file_info, path_from_root, file_id=None, format_label=True, is_version=True, logger=None):
    ...
    tei_metadata, version_name = _get_cached_metadata(file_info['full_path'], logger)
...
def _build_version_entry(file_info, path_from_root, file_id, format_label=True, logger=None):
    """Build a version entry with metadata."""
    return _build_entry_with_metadata(file_info, path_from_root, file_id, format_label, is_version=True, logger=logger)

def _build_gold_variants_array(file_id, file_type_data, logger=None):
    """
    ...
    for file_info in file_type_data['xml']:
        ...
        gold_entry = _build_entry_with_metadata(file_info, path_from_root, file_id, format_label=True, is_version=False, logger=logger)

        gold_variants.append(gold_entry)

    return gold_variants

def _add_top_level_metadata(file_dict, data_root, logger=None):
    ...
    if xml_path.startswith('/data/'):
        ...
        tei_metadata, _ = _get_cached_metadata(full_path, logger)
        if tei_metadata:
...
def _extract_variant_from_file(file_info, logger=None):
    """Extract variant ID from file metadata or filename."""
    # Try metadata first (using cache)
    tei_metadata, _ = _get_cached_metadata(file_info['full_path'], logger)
...
def _shorten_hashes(file_data, lookup_table, logger=None):
    ...
    if logger:
        logger.debug(f"Shortened hashes from 32 to {hash_length} characters ({len(all_hashes)} unique hashes)")

    return file_data, shortened_lookup

def _save_to_cache(file_path, data, logger=None):
...
def load_cached_file_data(db_dir: Path, logger=None):
    """
    ...
    try:
        ...
        if cache_age > 300:  # 5 minutes
            if logger:
                logger.debug("File cache is stale, will regenerate")
            return None
...
def get_file_data(data_root: Path, db_dir: Path, force_refresh=False, logger=None):
    ...
    if not force_refresh:
        cached_data = load_cached_file_data(db_dir, logger)
        if cached_data:
            if logger:
    ...
    if logger:
        logger.info("Generating fresh file data")
    return collect_and_cache_file_data(data_root, db_dir, logger)
