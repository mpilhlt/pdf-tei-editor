...
def resolve_hash_to_path(file_hash: str) -> str:
    """
    Resolve a file hash to its path using the lookup table.
    
    Args:
        file_hash (str): Hash to resolve
...
def add_path_to_lookup(file_path: str) -> str:
    ...
    with open(lookup_file, 'r+' if lookup_file.exists() else 'w+') as f:
        ...
        if new_short_hash not in lookup_table:
            ...
            current_app.logger.warning(f"Hash collision detected for {file_path} at length {hash_length}. Re-hashing lookup table.")
            ...
            current_app.logger.info(f"New safe hash length is {new_hash_length}.")
...
def generate_hashes_for_saved_file(pdf_absolute_path: str, tei_absolute_path: str, data_root: str) -> dict:

    """
    Generate shortened hashes for saved PDF and TEI files that match the current lookup table format.
    
    Args:
        pdf_absolute_path (str): Absolute path to PDF file
        tei_absolute_path (str): Absolute path to TEI file
