...
def get_lock_storage(data_root: Path, webdav_enabled: bool = False):
    # always use local storage at the moment
    return LocalLockStorage(data_root)

def get_lock_path(file_path, data_root: Path):
    ...
    storage = get_lock_storage(data_root)
...
def acquire_lock(file_path, session_id, data_root: Path, logger=None):
    """
    ...
    if logger:
        logger.debug(f"Acquiring lock for {file_path}")
    storage = get_lock_storage(data_root)
    ...
    try:
        ...
        if logger:
            logger.error(f"Could not create locks directory: {e}")
        raise RuntimeError(f"Could not create locks directory: {e}")

    lock_path = get_lock_path(file_path, data_root)
    ...
    try:
        ...
        if logger:
            logger.info(f"Lock acquired for {file_path} by session {session_id}")
        return True
        ...
        try:
            ...
            if existing_lock_id == session_id:
                ...
                if logger:
                    logger.info(f"Refreshed own lock for {file_path}")
                return True
            ...
            if not existing_lock_id or existing_lock_id =="" or is_lock_stale(storage.get_mtime(lock_path)):
                ...
                write_lock(overwrite=True)
                if logger:
                    logger.warning(f"Took over stale lock for {file_path} and session id {session_id} from session {existing_lock_id}")
                return True
            else:
                # Lock is held by another active session.
                if logger:
                    logger.warning(f"Failed to acquire lock for {file_path} and session id {session_id}. Held by {existing_lock_id}.")
                return False
        except Exception as e:
            raise RuntimeError(f"Error checking existing lock for {file_path}: {e}")

def release_lock(file_path, session_id, data_root: Path, logger=None):
    """
    ...
    try:
        if storage.exists(lock_path):
            ...
            if existing_lock_id == session_id:
                ...
                if logger:
                    logger.info(f"Lock released for {file_path} by session {session_id}")
                return {
                    "status": "success",
                    "action": "released",
            ...
            if logger:
                logger.info(f"Attempted to release lock for {file_path}, but no lock exists (idempotent success)")
                logger.debug(f"Session {session_id} release attempt on unlocked file - this may indicate upstream logic issues")
            return {
                "status": "success",
        ...
        if logger:
            logger.info(f"Attempted to release lock for {file_path}, but it was already gone.")
        return {
            "status": "success",
            "action": "already_released",
            "message": f"Lock was already released for {file_path}"
...
def purge_stale_locks(data_root: Path, logger=None):
        ...
        for lock in lock_files:
            if lock['type'] == 'file' and is_lock_stale(lock.get('modified')):
                try:
                    ...
                    if logger:
                        logger.info(f"Purged stale lock file: {lock['name']}")
                    purged_count += 1
                except Exception as e:
                    if logger:
                        logger.error(f"Failed to purge stale lock {lock['name']}: {e}")
    except FileNotFoundError:
        # No locks directory, nothing to purge.
        pass
    except Exception as e:
        if logger:
            logger.error(f"Error listing or purging stale locks: {e}")

    return purged_count

def get_all_active_locks(data_root: Path, logger=None):
        ...
        for lock in lock_files:
            if lock['type'] == 'file' and not is_lock_stale(lock.get('modified')):
                try:
                    ...
                    if logger:
                        logger.error(f"Error reading lock file {lock['name']}: {e}")
    except FileNotFoundError:
        if logger:
            logger.warning(f"Could not find locks directory")
        return {}
    except Exception as e:
        raise e
...
def check_lock(file_path, session_id, data_root: Path, logger=None):
    ...
    active_locks = get_all_active_locks(data_root, logger)

    if file_path in active_locks and active_locks[file_path] != session_id:
        if logger:
            logger.debug(f"File is locked by another session: {active_locks[file_path]}")
