...
- **Backend plugin tests location** - Plugin tests MUST be placed in the plugin's `tests/` directory (e.g., `fastapi_app/plugins/<plugin-name>/tests/`). This keeps tests colocated with the plugin code. The runner depends on the test file type: JS API tests (`*.test.js`) run via `node tests/backend-test-runner.js --test-dir fastapi_app/plugins/<plugin-name>/tests`; Python unit tests (`test_*.py`) run via `uv run python tests/unit-test-runner.py fastapi_app/plugins/<plugin-name>/tests` - `backend-test-runner.js` only discovers `*.test.js` files and silently reports "No test files found" for a directory containing only Python tests
