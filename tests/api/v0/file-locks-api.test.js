...
describe('File Locks API E2E Tests', { concurrency: 1 }, () => {
  ...
  async function cleanupSessionLocks() {
    ...
    try {
      ...
      logger.info(`Found ${lockedFileIds.length} total locked files`);
      ...
      const releasePromises = lockedFileIds.map(async fileId => {
        try {
          ...
          if (error.message.includes('409') || error.message.includes('CONFLICT')) {
            logger.warn(`Skipped lock for ${fileId} (owned by other session)`);
      ...
      logger.info(`Released ${successful} locks, skipped ${skipped} (other sessions), ${failed} failed`);
      ...
      if (remainingFileIds.length === expectedRemaining) {
        logger.success(`Session lock cleanup completed (${remainingFileIds.length} locks remain from other sessions)`);
      } else {
        logger.warn(`Warning: Expected ${expectedRemaining} remaining locks, but found ${remainingFileIds.length}`);
      }
  ...
  test('GET /api/files/locks should return active locks', async () => {
    ...
    logger.success(`Found ${fileIds.length} total active file locks`);
  });
  ...
  test('POST /api/files/check_lock should consistently report lock status', async () => {
    ...
    logger.success('Lock status correctly reported for owner session');
  });
