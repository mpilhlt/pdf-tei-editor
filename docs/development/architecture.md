...
Plugins are collected in `app/src/plugins.js`. Class-based plugins are also registered in `app/src/plugin-registry.js`, which is auto-generated by running `node scripts/build/build.js --steps=plugins`.
...
**Build steps** (`scripts/build/build.js`):
...
- Bundled into `templates.json` by `scripts/build/bundle-templates.js`
