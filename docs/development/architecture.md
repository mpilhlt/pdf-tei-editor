...
Plugins are collected in `app/src/plugins.js`. Class-based plugins are also registered in `app/src/plugin-registry.js`, which is auto-generated by running `node bin/build.js --steps=plugins`.
