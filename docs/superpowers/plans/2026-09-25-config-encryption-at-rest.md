...
- [ ] **Step 1: Write the failing test**
  ...
  (`bin/migrate-config-encrypt-secrets.py` has a hyphenated filename, like the other `bin/migrate-*.py` scripts, so it must be imported via `importlib.import_module` rather than a normal `import` statement — mirror however the existing `bin/migrate-tei-flavor-rename.py` is imported by its own tests, if it has any, for consistency.)
...
- [ ] **Step 3: Manual verification** — cannot be automated in this session; ask the user to confirm:
  ...
  - Make a live KISSKI/OpenAI-compatible request (e.g. via the app's inference settings UI, or `node bin/debug-api.js`) and confirm the provider still authenticates successfully — i.e. `config.get()` is still handing the real decrypted key to `OpenAICompatibleProvider`.
