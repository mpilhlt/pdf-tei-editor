...
class AccessControlChecker:
    ...
    def check_document_access(permissions: DocumentPermissions, user: Optional[Dict],
                            required_access: str = 'read', logger=None) -> bool:
        """
    ...
    def update_document_permissions(xml_content: str, new_visibility: str,
        ...
        try:
            ...
            if revision_desc is not None:
                ...
                from .tei_utils import serialize_tei_with_formatted_header
...
def get_document_permissions(file_path: str, data_root: Path, logger=None) -> DocumentPermissions:
    ...
    try:
        ...
        full_path = data_root / safe_file_path(file_path)
        if logger:
...
def check_file_access(file_path: str, user: Optional[Dict], data_root: Path, required_access: str = 'read', logger=None) -> bool:
    ...
    permissions = get_document_permissions(file_path, data_root, logger)
    result = AccessControlChecker.check_document_access(permissions, user, required_access, logger)
    if logger:
        logger.debug(f"ACCESS CONTROL DEBUG: check_file_access result={result}")
    return result

def check_file_access_from_metadata(metadata: Dict, user: Optional[Dict], required_access: str = 'read', logger=None) -> bool:
    ...
    return AccessControlChecker.check_document_access(permissions, user, required_access, logger)